...
export default async function AdminCouponsPage() {
  ...
  const [
    ...
    supabase
      .from("coupons")
      .select(
        "id, code, description, discount_type, discount_value, parent_id, class_id, program_id, pool_pass_id, private_lesson_id, starts_on, ends_on, max_uses, used_count, is_active"
      )
    ...
    supabase.from("private_lessons").select("id, title").order("title"),
  ]);
  ...
  const options: CouponOptions = {
    ...
    poolPasses: (passes ?? []).map((item) => ({
      id: item.id,
      label: item.title,
    })),
    privateLessons: (privateLessons ?? []).map((item) => ({
      id: item.id,
      label: item.title,
    })),
  };
