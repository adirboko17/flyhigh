...
import {
  ...
  businessDocCategoryLabel,
  createBusinessDocumentSignedUrl,
  deleteBusinessDocumentFile,
  formatFileSize,
  uploadBusinessDocumentFile,
  validateBusinessDocumentFile,
  type BusinessDocCategory,
} from "@/lib/storage/businessDocuments";
...
export type BusinessDocumentRow = Pick<
  BusinessDocument,
  | "id"
  | "title"
  | "category"
...
function resetFormState() {
  return {
    title: "",
    category: "osek" as BusinessDocCategory,
    notes: "",
...
export function BusinessDocumentsPanel({
  ...
  const [category, setCategory] = useState<BusinessDocCategory>("osek");
  ...
  const groups = useMemo(() => {
    return BUSINESS_DOC_CATEGORIES.map((item) => ({
      ...item,
      documents: documents.filter((doc) => doc.category === item.value),
  ...
  function pickFile(next: File | null, input?: HTMLInputElement | null) {
    ...
    const validationError = validateBusinessDocumentFile(next);
    if (validationError) {
  ...
  async function openDocument(doc: BusinessDocumentRow) {
    setOpeningId(doc.id);
    setError(null);
    const supabase = createClient();
    const result = await createBusinessDocumentSignedUrl(supabase, doc.file_path);
    setOpeningId(null);
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    const uploaded = await uploadBusinessDocumentFile(supabase, file);
    ...
    if (insertError) {
      await deleteBusinessDocumentFile(supabase, uploaded.path);
      setBusy(false);
  ...
  async function handleDelete(doc: BusinessDocumentRow) {
    ...
    await deleteBusinessDocumentFile(supabase, doc.file_path);
    setBusy(false);
    router.refresh();
  }
  ...
  return (
    <Card>
      <CardHeader>
        <div>
          <CardTitle>מסמכי העסק</CardTitle>
          <p className="mt-1 text-sm text-ink-500">
            תעודת עוסק, אישור ניהול חשבון ושאר מסמכים חשובים — במקום אחד.
          </p>
        </div>
      ...
      <div className="space-y-4 px-4 py-4 sm:px-5">
        ...
        {documents.length === 0 ? (
          <EmptyState
            ...
            description="העלו תעודת עוסק, אישור ניהול חשבון או כל מסמך עסקי שתרצו לשמור במקום מסודר."
            action={uploadButton}
                    ...
                    >
                      <div className="min-w-0 space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Badge tone="brand">
                            {businessDocCategoryLabel(doc.category)}
                          </Badge>
                        </div>
      ...
      <Modal
        ...
        title="העלאת מסמך עסק"
        description="המסמך יישמר במאגר הפרטי של העסק ויהיה זמין רק למנהלים."
      >
        <form onSubmit={handleUpload} className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="כותרת" required>
              <Input
                ...
                placeholder="לדוגמה: תעודת עוסק מורשה"
                disabled={busy}
                required
              />
            </Field>
            <Field label="סוג מסמך">
              <Select
                value={category}
                onChange={(e) =>
                  setCategory(e.target.value as BusinessDocCategory)
                }
                disabled={busy}
              >
                {BUSINESS_DOC_CATEGORIES.map((item) => (
          ...
          <Field label="הערה (אופציונלי)">
            <Textarea
              ...
              placeholder="תוקף, מספר תיק או פרטים נוספים..."
              disabled={busy}
              rows={2}
            />
          </Field>
