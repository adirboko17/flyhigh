...
export type CollectionCharge = {
  ...
  subject: string;
  subjectType: "class" | "program" | "pool_pass" | null;
...
export function CollectionsList({ parents }: CollectionsListProps) {
  ...
  const visibleParents = useMemo(() => {
    ...
    return parents
      .map((parent) => {
        const parentMatches =
          q === "" || normalize(parent.name).includes(q) || normalize(parent.phone).includes(q);
        ...
        const charges = parent.charges.filter((charge) => {
          ...
          if (methodFilter.length > 0 && !methodFilter.includes(charge.method)) return false;
          if (q === "") return true;
          return (
            parentMatches ||
        ...
        <div className="space-y-4">
          {visibleParents.map((parent) => (
            <ParentCard
              ...
              busyId={busyId}
              disabled={isPending}
              onToggleCharge={(charge) =>
                runAction(charge.id, () =>
                  setChargePaid({ paymentId: charge.id, paid: isOpen(charge) })
                )
              }
              onSettleAll={() =>
...
function ParentCard({
  ...
  busyId: string | null;
  disabled: boolean;
  onToggleCharge: (charge: CollectionCharge) => void;
  onSettleAll: () => void;
}) {
        ...
        <div className="flex w-full flex-wrap items-center gap-3 sm:w-auto sm:gap-4">
          ...
          {openCharges.length > 1 && (
            <Button
              ...
              onClick={onSettleAll}
            >
              {busyId === `parent:${parent.id}` ? "מסמן..." : "סימון הכל כשולם"}
            </Button>
          )}
        </div>
      </div>
      ...
      <ul className="divide-y divide-ink-100">
        {parent.charges.map((charge) => (
          <ChargeRow
            key={charge.id}
            charge={charge}
            busy={busyId === charge.id}
            disabled={disabled}
            onToggle={() => onToggleCharge(charge)}
          />
        ))}
      </ul>
    </Card>
...
function ChargeRow({
  ...
  return (
    <li className="flex flex-wrap items-center gap-x-4 gap-y-3 px-4 py-3.5 sm:px-5">
      ...
      <Badge tone={open ? "danger" : "success"}>{open ? "לא שולם" : "שולם"}</Badge>
