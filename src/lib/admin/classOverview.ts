...
export async function loadClassOverview(): Promise<ClassOverviewData> {
  ...
  const [
    ...
    supabase
      .from("enrollments")
      .select(
        "id, class_id, weekly_slot_id, session_id, is_trial, admin_assigned, status, payment_status, children(full_name), profiles(full_name, phone), payments(status, payment_method, external_reference, office_collection), class_sessions(session_date, start_time, end_time)"
      )
      .eq("type", "class")
