...
export async function loadCollectionParents(): Promise<CollectionParent[]> {
  ...
  for (const charge of charges ?? []) {
    ...
    if (existing) {
      ...
      byParent.set(charge.parent_id, {
        ...
        receiptIdNumber: charge.profiles?.receipt_id_number ?? null,
        adminNote: (() => {
